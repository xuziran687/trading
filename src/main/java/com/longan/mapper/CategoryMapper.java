...
public interface CategoryMapper extends BaseMapper<Category> {
    ...
    @Select("select id, parent_id, name, sort from category where parent_id = 0 order by sort ")
    List<CategoryVO> getLevel1();

    @Select("select id, parent_id, name, sort from category where parent_id = #{parentId} order by sort ")
