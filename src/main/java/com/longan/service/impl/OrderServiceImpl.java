...
import com.longan.mapper.OrderAddressMapper;
...
import com.longan.pojo.entity.OrderAddress;
...
public class OrderServiceImpl implements OrderService {
    ...
    public PageResult<MyOrderVO> getMyOrder(Integer role, Integer status, Integer page, Integer size) {
        ...
        List<MyOrderVO> voList = orderPage.getRecords().stream().map(order -> {
            ...
            GoodsImage image = goodsImageMapper.selectOne(new LambdaQueryWrapper<GoodsImage>().eq(GoodsImage::getGoodsId, order.getGoodsId()).eq(GoodsImage::getSort, 1));
            if (image != null) {
                // 图片
